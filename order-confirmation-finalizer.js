...
// complete draft -> ask confirmation -> explicit confirmation -> save.
// Never let Gemini's free-form reply create/confirm an order by itself.
const fs = require('fs');
...
Module._extensions['.js'] = function orderVerificationAccuracyLoader(module, filename) {
  ...
  const messages = all.filter(m => m.role === 'user');
  ...
  // Plain answers to the bot's immediately preceding question are valid.
  ...
  const confirmationOnly = /^(হ্যাঁ|জি|জ্বি|ঠিক আছে|ঠিক|কনফার্ম|কনফার্ম করুন|confirm|confirmed|নিশ্চিত|নিশ্চিত করছি|অর্ডার দিন|অর্ডার করুন|অর্ডারটা করে দিন|করে দিন|করে দেন)[\\s,।.!?]*$/i;

  let order_details = '';
  for (const text of userTexts) {
    if (inquiryOnly.test(text) || confirmationOnly.test(text)) continue;
    if (productPattern.test(text) && quantityPattern.test(text)) order_details = text;
  }
  if (!order_details) {
    const productText = userTexts.find(text => productPattern.test(text) && !inquiryOnly.test(text) && !confirmationOnly.test(text));
    const quantityText = userTexts.find(text => quantityPattern.test(text) && !confirmationOnly.test(text));
  ...
  if (!customer_name || !customer_address || !customer_phone || !order_details) {
    return { complete: false, customer_name, customer_address, customer_phone, order_details };
  }
...
function customerCancelledOrder(historyArr) {
  ...
  return /^(না|না লাগবে না|বাতিল|ক্যানসেল|cancel|বাদ দিন|বাদ দেন|অর্ডার করবেন না)[\\s,।.!?]*$/i.test(latest);
}
...
`;
  ...
  // Facebook: the save operation is impossible without explicit confirmation.
  const fbWebhookPos = source.indexOf('app.post("/webhook/facebook"');
  if (fbWebhookPos < 0) throw new Error('Order confirmation patch: Facebook webhook not found');

  const fbStart = source.indexOf('const banglaToEnglishDigits = text.replace(/[০-৯]/g, d => \'০১২৩৪৫৬৭৮৯\'.indexOf(d));', fbWebhookPos);
  ...
  const fbGate = `const facebookOrderDraft = await extractOrderInfo(page.histories[senderId]);
        ...
        console.log('Order gate FB:', JSON.stringify({ complete: facebookOrderDraft.complete, confirmed: facebookConfirmed, chatId: senderId }));
        if (!page.orderSaved[senderId] && facebookConfirmed && facebookOrderDraft.complete) {
          ...
          console.log('Order save result FB:', saved);
          if (saved) {
  ...
  // Telegram gate.
  const tgWebhookPos = source.indexOf('app.post("/telegram/webhook/');
  const tgStart = tgWebhookPos >= 0 ? source.indexOf('const banglaToEnglishDigits = text.replace(/[০-৯]/g, d => \'০১২৩৪৫৬৭৮৯\'.indexOf(d));', tgWebhookPos) : -1;
  if (tgStart >= 0) {
  ...
  // CRITICAL: override Gemini immediately before the actual Facebook send.
  // We use the current raw customer message as an additional guard so that a
  // complete draft can never be reported as confirmed just because Gemini said so.
  const replyAnchor = `let reply = data?.candidates?.[0]?.content?.parts?.[0]?.text || "দুঃখিত, উত্তর তৈরি করা যায়নি।";`;
  const fbReplyPos = source.indexOf(replyAnchor, fbWebhookPos);
  if (fbReplyPos < 0) throw new Error('Order confirmation patch: Facebook reply anchor not found');
  const fbReplyPatch = `${replyAnchor}
        const latestFacebookText = String(text || '').trim();
        const facebookDraftForReply = await extractOrderInfo(page.histories[senderId]);
        const latestFacebookConfirmed = customerConfirmedOrder(page.histories[senderId]);
        if (facebookDraftForReply.complete && !latestFacebookConfirmed && !customerCancelledOrder(page.histories[senderId])) {
          console.log('Order confirmation guard FB: replacing AI confirmation with confirmation question');
  ...
  console.log('✓ Order verification finalizer active');
